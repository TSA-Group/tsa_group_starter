...
export default function EventsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if(selectedActivities.length>0) list=list.filter(e=>selectedActivities.every(a=>e.activities.includes(a)));
    if(query) list=list.filter(e=>`${e.title} ${e.description} ${e.location}`.toLowerCase().includes(query.toLowerCase()));
    if(sortBy==="popular") list=[...list].sort((a,b)=>b.attendees-a.attendees);
    else list=[...list].sort((a,b)=>new Date(a.date).getTime()-new Date(b.date).getTime());
    return list;
  },[selectedCategory,selectedActivities,query,sortBy]);

  const toggleActivity=(id:string)=>setSelectedActivities(prev=>prev.includes(id)?prev.filter(x=>x!==id):[...prev,id]);

  return (
    <>
      <Head><title>Gatherly — Community Events</title></Head>
      <div className="min-h-screen bg-gradient-to-br from-[#071026] via-[#0b1220] to-[#020617] text-white antialiased">
        <div className="max-w-7xl mx-auto px-6 py-10">
          <header className="mb-8">
            ...
            <p className="mt-2 text-slate-400 max-w-2xl">Discover local volunteering opportunities and community events.</p>
          </header>
          ...
          <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-5 mb-10">
            <div className="flex flex-wrap gap-2 mb-4">{categories.map(c=>(
              <button key={c.id} onClick={()=>setSelectedCategory(c.id)} className={`px-4 py-2 rounded-full text-sm transition ${selectedCategory===c.id?"bg-indigo-500 text-white":"bg-white/10 text-slate-300 hover:bg-white/20"}`}>{c.name}</button>
            ))}</div>

            <div className="flex flex-wrap gap-2 mb-4">{activities.map(a=>(
              <button key={a.id} onClick={()=>toggleActivity(a.id)} className={`px-4 py-2 rounded-full text-sm transition ${selectedActivities.includes(a.id)?"bg-indigo-500 text-white":"bg-white/10 text-slate-300 hover:bg-white/20"}`}>{a.name}</button>
            ))}</div>

            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search events..." className="flex-1 bg-white/5 placeholder:text-slate-400 text-slate-100 px-4 py-2 rounded-xl border border-white/10 focus:outline-none focus:ring-2 focus:ring-indigo-500"/>
              <div className="flex items-center gap-2">
                ...
                <div className="inline-flex rounded-full bg-white/10 p-1 border border-white/10">
                  {["upcoming","popular"].map(option=>(
                    <button key={option} onClick={()=>setSortBy(option as "upcoming"|"popular")} className={`px-4 py-1 text-sm rounded-full transition ${sortBy===option?"bg-indigo-500 text-white":"text-slate-200 hover:bg-white/10"}`}>{option==="upcoming"?"Upcoming":"Popular"}</button>
                  ))}
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">{filtered.map(ev=>{
            const percent=Math.round((ev.attendees/ev.spots)*100);
            const spotsLeft=ev.spots-ev.attendees;
            return (
              <article key={ev.id} className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-6 hover:border-indigo-400/40 hover:shadow-xl transition">
                ...
                <div className="flex gap-2 mt-5">
                  <Link href={`/events?id=${ev.id}`} className="flex-1 text-center bg-indigo-500 hover:bg-indigo-600 text-white py-2 rounded-xl">Register</Link>
                </div>
              </article>
            )
          })}</div>
