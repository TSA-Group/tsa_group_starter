...
export default function Home() {
  ...
  useEffect(() => {
    return scrollY.onChange((y) => {
      ORBS.forEach((orb, i) => {
        ...
        orbX[i].set(
          (mousePos.x / (window.innerWidth || 1) - 0.5) * 200 * orb.speed,
        );
      });
    });
  ...
  const background = useTransform(
    scrollY,
    [0, scrollRange * 0.3, scrollRange],
    ["#ffffff", "#EEF4FA", "#E5E9EF"],
  );
  const greyOverlay = useTransform(
    scrollY,
    [scrollRange * 0.3, scrollRange * 0.6],
    ["rgba(226,232,240,0)", "rgba(203,213,225,0.65)"],
  );
  const blueOverlay = useTransform(
    scrollY,
  ...
  return (
    ...
    >
      ...
      />
      <motion.div
        style={{ backgroundColor: blueOverlay }}
        className="fixed inset-0 pointer-events-none -z-20"
      />
      ...
      <motion.header
        ...
        className="min-h-[95vh] relative flex flex-col justify-center max-w-7xl mx-auto px-6"
      >
        ...
        <motion.h1
          ...
          className="text-8xl sm:text-9xl font-extrabold tracking-tight text-transparent bg-clip-text bg-gradient-to-br from-blue-950 via-blue-800 to-blue-600 text-center"
        >
          GATHERLY
        </motion.h1>
        <motion.p
          style={{ y: heroTextY }}
          className="mt-10 max-w-xl text-xl text-blue-800 text-center mx-auto"
        >
          Discover, connect, and engage with your real community — without the
          noise.
        </motion.p>
      </motion.header>
      ...
      <motion.main className="max-w-7xl mx-auto px-6 pb-32 flex flex-col lg:flex-row gap-10 lg:gap-14 mt-24">
        ...
        >
          <motion.div
            ...
            layout
            variants={cardPop}
            className="bg-white rounded-3xl border border-blue-200 shadow-lg p-6"
          >
            {/* CALENDAR HEADER */}
                      ...
                      {events
                        ...
                        .map((event, i) => (
                          <motion.li
                            ...
                            className="border-l-4 border-blue-500 pl-3 cursor-pointer transition-transform"
                          >
        ...
        return (
          <React.Fragment key={i}>
            <motion.section
              initial="hidden"
              variants={container}
              className={`max-w-7xl mx-auto px-6 my-24`}
            >
              <motion.div
                variants={fadeUp}
                initial="hidden"
              ...
              >
                <Link href="#" className="block w-full md:w-[48%]">
                  <motion.div
                    ...
                    className="bg-white rounded-3xl border border-blue-200 shadow-lg overflow-hidden cursor-pointer"
                  >
      ...
      <section className="w-full mt-40 mb-40 px-6">
        ...
        <div className="w-full max-w-4xl mx-auto p-10 rounded-3xl shadow-lg text-blue-900 bg-gradient-to-br from-blue-50 via-blue-100 to-blue-50 border border-blue-200">
          <p className="mb-6">
            ...
            The initial concept for Gatherly was formed to give communities a
            single place to connect.
          </p>
          <p className="mb-6">
            <span className="font-bold text-blue-700">
              2024 – Building the Platform:
            </span>{" "}
            Core layouts, animations, and interactive features were developed.
          </p>
          <p className="mb-6">
            <span className="font-bold text-blue-700">
              2025 – Public Launch:
            </span>{" "}
            Gatherly launched with events, calendars, and community tools.
          </p>
          <p>
            <span className="font-bold text-blue-700">Looking Ahead:</span>{" "}
            Expanding neighborhoods, stories, and ways for people to get
            involved.
          </p>
        </div>
      </section>
