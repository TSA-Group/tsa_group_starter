...
const fadeUp: Variants = { hidden: { opacity: 0, y: 18, filter: "blur(4px)" }, show: { opacity: 1, y: 0, filter: "blur(0px)", transition: { duration: 0.6, ease: "easeOut" } } };
const cardPop: Variants = { hidden: { opacity: 0, y: 10, scale: 0.98 }, show: { opacity: 1, y: 0, scale: 1, transition: { duration: 0.55, ease: "easeOut" } } };
    ...
    <motion.div style={{ background }} className="min-h-screen px-4 sm:px-6 lg:px-8 pb-32">
          ...
          >
            ...
            <div className="grid grid-cols-7 gap-1">
              {calendarDays.map((date, idx) => {
                ...
                return (
                  <div key={idx} className={`relative flex flex-col items-center justify-center h-12 sm:h-14 w-full rounded-lg text-sm sm:text-base font-semibold cursor-pointer transition-colors ${dayClasses(date, isToday, isSelected)}`}
                    onClick={() => setSelectedDate(prev => prev && prev.getTime() === date.getTime() ? null : date)}>
                    <span>{date.getDate()}</span>

                    {/* Dot linking to /events */}
                    {dayEvents.length > 0 && (
                      <span
                        className="mt-1 h-2 w-2 rounded-full cursor-pointer"
                        style={{ backgroundColor: categoryColors[dayEvents[0].category] }}
                        onClick={() => {
                          const dateParam = `${date.getFullYear()}-${(date.getMonth()+1).toString().padStart(2,"0")}-${date.getDate().toString().padStart(2,"0")}`;
                          router.push(`/events?date=${dateParam}`);
                        }}
                      />
                    )}
      ...
      <motion.section initial="hidden" variants={container} className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-24 mb-32 space-y-16">
        {[{
          ...
          <motion.div key={i} variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true, margin: "-100px" }}
            className={`flex ${item.align === "right" ? "justify-end" : "justify-start"}`}>
            <Link href={item.href} className="block w-full md:w-[48%]">
              <motion.div whileHover={{ y: -8, scale: 1.02 }} transition={{ type: "spring", stiffness: 260, damping: 18 }}
      ...
      <section className="relative w-full mt-40 mb-32 px-6">
        ...
        <div className="w-full max-w-4xl mx-auto p-10 rounded-2xl shadow-xl border border-blue-200" style={{ background: "linear-gradient(to bottom right, #DBEAFE, #E0F2FE)" }}>
          {[{ year: 2020, desc: "Gatherly founded by local volunteers to strengthen neighborhood bonds." },
            { year: 2021, desc: "Hosted first community dinner with over 100 participants." },
            { year: 2022, desc: "Launched neighborhood clean-up program." },
            { year: 2023, desc: "Expanded volunteer network across 3 cities." }].map((item, i) => (
            <div key={i} className="mb-6 last:mb-0">
              <h3 className="text-xl font-semibold text-blue-900">{item.year}</h3>
      ...
      <section className="max-w-7xl mx-auto px-6 mb-32">
        <motion.div variants={fadeUp} initial="hidden" whileInView="show" className="rounded-2xl p-10 shadow-xl border border-blue-200 text-center" style={{ background: "linear-gradient(to top right, #E0F2FE, #DBEAFE)" }}>
          <h2 className="text-3xl font-extrabold text-blue-900 mb-4">Our Mission</h2>
          <p className="text-blue-700 max-w-2xl mx-auto">To bring neighbors together, celebrate local stories, and create opportunities for everyone to make a positive impact in our communities.</p>
        </motion.div>
      </section>

      {/* FOOTER */}
      <footer className="w-full border-t border-blue-200 py-10 mt-32 text-center text-blue-700">
        <p>© {year} Gatherly. All rights reserved.</p>
        <div className="mt-2 space-x-4">
          <Link href="/privacy" className="hover:text-blue-900">Privacy Policy</Link>
          <Link href="/terms" className="hover:text-blue-900">Terms of Service</Link>
        </div>
      </footer>
    </motion.div>
  );
